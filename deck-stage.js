...
(() => {
  ...
  const stylesheet = `
    ...
    :host([presentation]) .rail,
    :host([presentation]) .rail-resize,
    :host([presentation]) .overlay,
    :host([presentation]) .tapzones,
    :host([presentation]) .ctxmenu,
    :host([presentation]) .confirm-backdrop { display: none !important; }
  ...
  class DeckStage extends HTMLElement {
    static get observedAttributes() { return ['width', 'height', 'noscale', 'no-rail', 'presentation']; }
    ...
    connectedCallback() {
      ...
      if (params.get('present') === '1' || params.get('final') === '1') {
        this.setAttribute('no-rail', '');
        this.setAttribute('presentation', '');
    ...
    _onHashChange() {
      const h = (location.hash || '').match(/^#\/?(\d+)$/);
      if (!h || !this._slides.length) return;
      const n = parseInt(h[1], 10) - 1;
      if (n < 0 || n >= this._slides.length || n === this._index) return;
      this._index = n;
      this._applyIndex({ showOverlay: false, broadcast: true, reason: 'api' });
    }
    ...
    _flashOverlay() {
      ...
      if (!this._overlay || this._presenting || this.hasAttribute('presentation')) return;
    ...
    _railWidth() {
      ...
      if (!this._railEnabled || !this._railVisible || this.hasAttribute('no-rail')
          || this.hasAttribute('noscale') || this.hasAttribute('presentation')
          || this._presenting || this._previewMode) return 0;
    ...
    _onMouseMove() {
      if (this.hasAttribute('presentation')) return;
    ...
    _syncRailHidden() {
      ...
      // :host([data-rail-anim]).
      const hard = !this._railEnabled || this._presenting || this._previewMode || this.hasAttribute('presentation');
    ...
    _materialize(entry) {
      ...
      const slideStyle = getComputedStyle(entry.slide);
      const themeProps = [
        '--slide-bg',
        '--stage-a',
        '--stage-b',
      ...
      themeProps.forEach((prop) => {
        const value = slideStyle.getPropertyValue(prop);
        if (value) clone.style.setProperty(prop, value.trim());
      });
      const sourceInner = entry.slide.querySelector(':scope > .slide-inner');
      const cloneInner = clone.querySelector(':scope > .slide-inner');
      if (sourceInner && cloneInner) {
        const sourceInnerStyle = getComputedStyle(sourceInner);
        cloneInner.style.padding = sourceInnerStyle.padding;
      }
