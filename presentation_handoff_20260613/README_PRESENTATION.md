...
3. `광양 B3 01 발표 세트 실행.bat`을 실행합니다.
...
로컬 백업 발표에서는 `local_fallback/local-slide-sync.cjs`가 싱크 서버 역할을 합니다. `광양 B3 04 로컬 백업 실행.bat`이 자동으로 실행하므로 직접 열 필요는 없습니다.
...
발표가 끝나면 `광양 B3 03 발표 세트 종료.bat`을 실행합니다.
