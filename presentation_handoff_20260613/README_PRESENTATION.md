...
4. `04_START_LOCAL_BACKUP.bat`
   ...
   - Node.js가 설치되어 있지 않아도 ZIP 안의 휴대용 Node를 먼저 사용합니다.
5. `05_STOP_LOCAL_BACKUP.bat`
   - 로컬 백업 발표가 끝난 뒤 누릅니다.
   - 로컬 브라우저 창과 로컬 서버를 정리합니다.
6. `06_RETURN_DUPLICATE_MODE.bat`
   - 우리 발표가 끝난 뒤 다음 발표자를 위해 누릅니다.
   - 온라인/로컬 발표용 창과 서버를 정리하고 Windows 화면 모드를 **복제**로 되돌립니다.
...
1. `gwangyang_b3_presentation_20260613.zip`을 먼저 압축 해제합니다.
2. 풀린 `gwangyang_b3_presentation_20260613` 폴더를 엽니다.
3. `04_START_LOCAL_BACKUP.bat`을 실행합니다.
4. 공유용 슬라이드 창은 발표장/Zoom 공유 화면에 두고, 발표자 콘솔 창은 노트북 화면에 둡니다.
5. 발표가 끝나면 `06_RETURN_DUPLICATE_MODE.bat`을 실행합니다.

로컬 실행은 인터넷 없이 슬라이드와 콘솔을 띄우는 방식입니다. Node.js가 설치되어 있지 않아도 ZIP 안의 `local_fallback/portable-node/node.exe`를 먼저 사용합니다.

창 위치는 실행 파일이 자동으로 맞춥니다. 발표장 화면 연결 직후에는 Windows가 확장 모드 전환을 몇 초 늦게 반영할 수 있어 실행 파일이 5초 기다린 뒤 창을 엽니다. 그래도 반대로 뜨면 해당 창을 선택하고 `Windows + Shift + 왼쪽/오른쪽 화살표`로 다른 화면에 보낼 수 있습니다.
